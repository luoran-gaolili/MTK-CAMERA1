...
package com.mediatek.camera.v2.ui;
...
public class CountDownView extends FrameLayout {
    private static final Tag TAG = new Tag(CountDownView.class.getSimpleName());
    ...
    /**
     * Listener that gets notified when the countdown status has been updated
     * (i.e. remaining seconds changed, or finished).
     */
    public interface OnCountDownStatusListener {
        /**
         * Gets notified when the remaining seconds for the countdown has
         * changed.
         *
         * @param remainingSeconds
         *            seconds remained for countdown
         */
    ...
    public void onPreviewAreaChanged(RectF previewArea) {
        LogHelper.i(TAG, "onPreviewAreaChanged " + previewArea);
    ...
    private void startFadeOutAnimation() {
        ...
        float endScale = 2.5f;
        mRemainingSecondsView.animate().scaleX(endScale).scaleY(endScale).alpha(0f)
    ...
    /**
     * Sets a listener that gets notified when the status of countdown has
     * changed.
     */
    ...
    public void startCountDown(int sec) {
        if (sec <= 0) {
            LogHelper.w(TAG, "Invalid input for countdown timer: " + sec + " seconds");
            return;
        }
        if (isCountingDown()) {
            cancelCountDown();
