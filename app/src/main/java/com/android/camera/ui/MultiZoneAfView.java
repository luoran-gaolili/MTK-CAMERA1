...
public class MultiZoneAfView extends View {
    private static final String TAG = "MultiZoneAfView";
    ...
    public MultiZoneAfView(Context context, AttributeSet attr) {
        ...
        mScaleRatio = Float.parseFloat(SystemProperties.get(MZAF_INDICATOR_KEY,
    ...
    protected void onDraw(Canvas canvas) {
        if (mWindows != null && mWindows.length > 0) {
            Log.d(TAG, "onDraw length " + mWindows.length + " ,mDisplayOrientation = "
            ...
            for (int i = 0; i < mWindows.length; i++) {
                ...
                // multiafdebug
                Log.d(TAG, "window[ " + i + " ] result " + mWindows[i].mResult);
    ...
    private Drawable[] getViewDrawable() {
        ...
        for (int i = 0; i < length; i++) {
            mAfStatusIndicators[i] = mContext.getResources().getDrawable(MZAF_ICON[i]);
        }
    ...
    private Rect scale() {
        ...
        float left = centerX - width * mAnimatorRatio * mScaleRatio / 2;
        float top = centerY - height * mAnimatorRatio * mScaleRatio / 2;
        float right = centerX + width * mAnimatorRatio * mScaleRatio / 2;
        float bottom = centerY + height * mAnimatorRatio * mScaleRatio / 2;
